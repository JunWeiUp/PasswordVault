...
The configured delivery target is GitHub Releases for Android packages, Web assets, and the unpacked Chromium extension. There is no Cloudflare deployment configuration or hosted production endpoint in this repository. See [REGISTRY.md](REGISTRY.md) for build inputs, validation, and the distribution inventory; [DEVELOPMENT.md](DEVELOPMENT.md) covers local setup.
