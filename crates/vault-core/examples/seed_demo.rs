...
fn main() {
    ...
    session
        ...
        .unwrap();
    let notes=[("家庭重要资料","家庭","集中保存家庭常用信息，查找时更省心。\n\n## 紧急联系\n- 家庭邮箱：family@example.com\n- 物业服务：示例联系人\n\n## 重要文件\n- 证件与合同：书房文件柜 · 第二层\n- 房屋保险：电子保单已归档\n\n## 家庭网络\n- 网络名称：Home-Example\n- 账号与密码统一保存在「密码」中。\n\n## 备份提醒\n每月检查一次备份文件，更新过期信息。"),
    ...
    for (index, title) in ["家庭邮箱", "云盘", "购物账号", "工作协作", "示例服务"]
        .iter()
        .enumerate()
    {
        session.command(json!({"op":"save","item":{"id":format!("password-{index}"),"type":"password","title":title,"username":"family@example.com","password":format!("Synthetic-only-{index}!"),"url":"https://mail.example.com","category":"家庭","note":"用于家庭服务通知，不用于工作。","tags":["个人"]}}).to_string()).unwrap();
    }
    ...
    session.command(json!({"op":"settings","settings":{"language":"zh","theme":"light","autoLockMinutes":30}}).to_string()).unwrap();
