...
class VaultRepository {
      ...
      final decryptedBatch = await Future.wait(batch.map((row) async {
        ...
        if (row.sharedVaultId != null && userKeyPair != null) {
          if (sharedKeyCache.containsKey(row.sharedVaultId)) {
            ...
            final key = await getSharedVaultKey(row.sharedVaultId!, userKeyPair);
            if (key != null) {
        ...
        Future<String?> _safeDecrypt(String? encryptedData, String fieldName) async {
          ...
          try {
            ...
            debugPrint('❌ Decryption failed for field $fieldName in item ${row.id}: $e');
        ...
        decryptionTasks.add(_safeDecrypt(row.secret, 'secret').then((v) => decryptedSecret = v));
        decryptionTasks.add(_safeDecrypt(row.password, 'password').then((v) => decryptedPassword = v));
        decryptionTasks.add(_safeDecrypt(row.mnemonic, 'mnemonic').then((v) => decryptedMnemonic = v));
        decryptionTasks.add(_safeDecrypt(row.privateKey, 'privateKey').then((v) => decryptedPrivateKey = v));
        decryptionTasks.add(_safeDecrypt(row.address, 'address').then((v) => decryptedAddress = v));
        ...
        return VaultItem(
          ...
          tags: decryptedTags != null ? List<String>.from(jsonDecode(decryptedTags!)) : [],
  ...
  Future<void> addItem(
    ...
    if (item.sharedVaultId != null && userKeyPair != null) {
      ...
      if (role == null) throw Exception('您不是该共享库的成员');
      if (role == SharedMemberRole.viewer) throw Exception('您没有在该共享库中添加项的权限');
  ...
  Future<void> updateItem(
    ...
    // 确定旧数据的解密密钥
      ...
      if (item.sharedVaultId != null && userKeyPair != null) {
        ...
        final role = await _getMemberRole(item.sharedVaultId!, userKeyPair);
        if (role == null) throw Exception('您不是该共享库的成员');
        if (role == SharedMemberRole.viewer) throw Exception('您没有在该共享库中修改项的权限');
  ...
  Future<void> softDeleteItem(String id, {SimpleKeyPair? userKeyPair}) async {
    final item = await (_db.select(_db.vaultItems)..where((t) => t.id.equals(id))).getSingleOrNull();
    if (item != null && item.sharedVaultId != null && userKeyPair != null) {
      final role = await _getMemberRole(item.sharedVaultId!, userKeyPair);
      if (role == null) throw Exception('您不是该共享库的成员');
      if (role == SharedMemberRole.viewer) throw Exception('您没有在该共享库中删除项的权限');
    }
  ...
  Future<void> restoreItem(String id, {SimpleKeyPair? userKeyPair}) async {
    ...
    if (item != null && item.sharedVaultId != null && userKeyPair != null) {
      final role = await _getMemberRole(item.sharedVaultId!, userKeyPair);
      if (role == null) throw Exception('您不是该共享库的成员');
      if (role == SharedMemberRole.viewer) throw Exception('您没有在该共享库中恢复项的权限');
    }
  ...
  Future<void> createSharedVault({
    ...
    final encryptedVaultKeyForOwner = await _encryptionService.encryptWithPublicKey(
    ...
    await _db.transaction(() async {
      // 3. 插入共享库
      await _db.into(_db.sharedVaults).insert(SharedVaultsCompanion.insert(
        id: vaultId,
        name: name,
      ...
      // 4. 将所有者添加为成员
      await _db.into(_db.sharedMembers).insert(SharedMembersCompanion.insert(
        id: "${vaultId}_owner",
        vaultId: vaultId,
  ...
  Future<void> addMemberToSharedVault({
    ...
    final currentUserMember = await (_db.select(_db.sharedMembers)
          ..where((t) => t.vaultId.equals(vaultId))
          ..where((t) => t.userPublicKey.equals(base64.encode(currentUserPubKey.bytes))))
        .getSingleOrNull();
        
    if (currentUserMember == null) throw Exception('未找到成员记录');
    if (SharedMemberRole.values[currentUserMember.role] == SharedMemberRole.viewer) {
      throw Exception('权限不足，仅查看者无法添加成员');
    }
    
    final encryptedVaultKeyForMe = base64.decode(currentUserMember.encryptedVaultKey);
    final decryptedVaultKeyBytes = await _encryptionService.decryptWithPrivateKey(
      encryptedVaultKeyForMe,
      currentUserKeyPair,
    );
    ...
    final encryptedVaultKeyForMember = await _encryptionService.encryptWithPublicKey(
      utf8.encode(vaultKeyBase64),
  ...
  Future<void> removeMemberFromSharedVault({
    ...
    final currentUserPublicKey = base64.encode((await currentUserKeyPair.extractPublicKey()).bytes);
    final currentUserMember = await (_db.select(_db.sharedMembers)
          ..where((t) => t.vaultId.equals(vaultId))
          ..where((t) => t.userPublicKey.equals(currentUserPublicKey)))
        .getSingleOrNull();

    if (currentUserMember == null || SharedMemberRole.values[currentUserMember.role] != SharedMemberRole.owner) {
      throw Exception('只有所有者可以移除成员');
    }

    final targetMember = await (_db.select(_db.sharedMembers)..where((t) => t.id.equals(memberId))).getSingleOrNull();
    if (targetMember == null) throw Exception('未找到目标成员');
    if (SharedMemberRole.values[targetMember.role] == SharedMemberRole.owner) {
      throw Exception('无法移除所有者');
    }

    await (_db.delete(_db.sharedMembers)..where((t) => t.id.equals(memberId))).go();
  }
  ...
  Future<void> updateMemberRole({
    ...
    if (currentUserMember == null || SharedMemberRole.values[currentUserMember.role] != SharedMemberRole.owner) {
      throw Exception('只有所有者可以修改权限');
    }

    final targetMember = await (_db.select(_db.sharedMembers)..where((t) => t.id.equals(memberId))).getSingleOrNull();
    if (targetMember == null) throw Exception('未找到目标成员');
    if (SharedMemberRole.values[targetMember.role] == SharedMemberRole.owner) {
      throw Exception('无法修改所有者的权限');
    }

    await (_db.update(_db.sharedMembers)..where((t) => t.id.equals(memberId))).write(
      SharedMembersCompanion(role: Value(newRole.index)),
    );
  }
  ...
  Future<void> deleteSharedVault({
    ...
    if (currentUserMember == null || SharedMemberRole.values[currentUserMember.role] != SharedMemberRole.owner) {
      throw Exception('只有所有者可以删除共享库');
    }

    await _db.transaction(() async {
      await (_db.delete(_db.sharedVaults)..where((t) => t.id.equals(vaultId))).go();
      await (_db.delete(_db.sharedMembers)..where((t) => t.vaultId.equals(vaultId))).go();
      // 将所有属于该库的项设为未归属
      await (_db.update(_db.vaultItems)..where((t) => t.sharedVaultId.equals(vaultId))).write(
        const VaultItemsCompanion(sharedVaultId: Value(null)),
      );
    });
  }
  ...
  Future<void> rotateSharedVaultKey({
    ...
    if (currentUserMember == null || SharedMemberRole.values[currentUserMember.role] != SharedMemberRole.owner) {
      throw Exception('只有所有者可以轮转密钥');
    }

    // 2. 获取旧密钥以解密现有项
    final oldEncryptedKey = base64.decode(currentUserMember.encryptedVaultKey);
    final oldKeyBytes = await _encryptionService.decryptWithPrivateKey(oldEncryptedKey, currentUserKeyPair);
    ...
    // 4. 获取所有成员并为他们重新加密新密钥
    final members = await (_db.select(_db.sharedMembers)..where((t) => t.vaultId.equals(vaultId))).get();
    ...
    for (final member in members) {
      ...
      memberUpdates.add(SharedMembersCompanion(
        id: Value(member.id),
    ...
    // 5. 获取所有属于该库的项
    final rows = await (_db.select(_db.vaultItems)..where((t) => t.sharedVaultId.equals(vaultId))).get();
    ...
    for (final row in rows) {
      ...
      // 用新密钥重新加密
      final companion = await _buildInsertCompanion(decryptedItem, newSecretKey);
    ...
    await _db.transaction(() async {
      ...
      for (final update in memberUpdates) {
        await (_db.update(_db.sharedMembers)..where((t) => t.id.equals(update.id.value))).write(update);
      }

      // 更新库的密钥（存储的是所有者的版本）
      final encryptedNewKeyForOwner = await _encryptionService.encryptWithPublicKey(
        utf8.encode(newVaultKeyBase64),
        currentUserPubKey.bytes,
      );
      await (_db.update(_db.sharedVaults)..where((t) => t.id.equals(vaultId))).write(
        SharedVaultsCompanion(encryptedVaultKey: Value(base64.encode(encryptedNewKeyForOwner))),
      );

      // 更新所有项
      for (final update in itemUpdates) {
        await (_db.update(_db.vaultItems)..where((t) => t.id.equals(update.id.value))).write(update);
      }
    });
  }

  Future<VaultItem> _decryptVaultItemRow(VaultItemEntity row, SecretKey encryptionKey) async {
    String? decryptedSecret;
    String? decryptedPassword;
    String? decryptedMnemonic;
    String? decryptedPrivateKey;
  ...
  Future<List<SharedVault>> getSharedVaults(SimpleKeyPair userKeyPair) async {
    ...
    return rows.map((row) => SharedVault(
      id: row.id,
      name: row.name,
  ...
  Future<List<SharedMember>> getVaultMembers(String vaultId) async {
    final rows = await (_db.select(_db.sharedMembers)..where((t) => t.vaultId.equals(vaultId))).get();
    return rows.map((row) => SharedMember(
      id: row.id,
      vaultId: row.vaultId,
  ...
  Future<void> addSharedVaults(List<SharedVault> vaults) async {
    ...
    await _db.batch((batch) {
      batch.insertAll(
        _db.sharedVaults,
        vaults.map((v) => SharedVaultsCompanion.insert(
              id: v.id,
              name: v.name,
  ...
  Future<void> updateSharedVaultDiscoverable(String vaultId, bool isDiscoverable) async {
    await (_db.update(_db.sharedVaults)..where((t) => t.id.equals(vaultId))).write(
      SharedVaultsCompanion(isDiscoverable: Value(isDiscoverable)),
    );
  }
  ...
  Future<void> addSharedMembers(List<SharedMember> members) async {
    ...
    await _db.batch((batch) {
      batch.insertAll(
        _db.sharedMembers,
        members.map((m) => SharedMembersCompanion.insert(
              id: m.id,
              vaultId: m.vaultId,
  ...
  Future<SecretKey?> getSharedVaultKey(String sharedVaultId, SimpleKeyPair userKeyPair) async {
    final publicKey = await userKeyPair.extractPublicKey();
    final publicKeyBase64 = base64.encode(publicKey.bytes);

    final member = await (_db.select(_db.sharedMembers)
  ...
  Future<SharedMemberRole?> _getMemberRole(String sharedVaultId, SimpleKeyPair userKeyPair) async {
    final publicKey = await userKeyPair.extractPublicKey();
    final publicKeyBase64 = base64.encode(publicKey.bytes);
