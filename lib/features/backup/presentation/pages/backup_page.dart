                      ...
                      children: [
                        ...
                        Switch(
                          ...
                          onChanged: (v) =>
                              ref
                                      .read(
                                        encryptBackupToggleProvider.notifier,
                                      )
                                      .state =
                                  v,
                        ),
                            ...
                            ? () async {
                                try {
                                  await ref
                                      ...
                                      .performBackup(encrypt: shouldEncrypt);
                                  if (context.mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(
                                        content: Text(tr.backupComplete),
                                      ),
                                    );
                                  }
                                } catch (e) {
                                  if (context.mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(
                                        content: Text(tr.backupFailed(e)),
  ...
  Widget _buildHistoryItem(
    ...
    return Container(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          '${dateFormat.format(item.createdAt)}  ($sizeStr)',
        ...
        trailing: PopupMenuButton<String>(
            ...
            if (value == 'restore') {
              final confirmed = await showDialog<bool>(
                ...
                builder: (context) => AlertDialog(
                  title: Text(tr.restoreBackup),
                  content: Text(tr.mergeThisBackupByModificationTimeNewer),
                  actions: [
                    TextButton(
                ...
                try {
                  ...
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          tr.restoreCompleteAddedUpdatedSkipped(
                            result.added,
                            result.updated,
                            result.skipped,
                          ),
                  ...
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(tr.restoreFailed(e)),
                        backgroundColor: Colors.red,
                      ),
                    );
                  }
          ...
          itemBuilder: (context) => [
            PopupMenuItem(value: 'restore', child: Text(tr.restoreThisBackup)),
          ],
        ),
      ),
    );
