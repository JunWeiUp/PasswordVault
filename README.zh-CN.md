...
推送版本标签，或针对已有标签手动运行工作流，即可开始发布交付。流程核对版本和 Android 签名身份，将 APK/AAB、Web、扩展与构建元数据、SHA-256 校验文件打包，并 **直接发布普通 GitHub Release 并标为 Latest，不启用草稿或 Pre-release**。主分支推送生成 CI 构建产物；发布下载包需要推送新的版本标签。签名密钥、产物和审核清单见[发布指南](docs/RELEASING.md)。
